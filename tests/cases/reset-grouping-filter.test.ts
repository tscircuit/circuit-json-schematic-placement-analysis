...
test("does not mistake an ADC filter on a net named RESET for a reset network", async () => {
  const circuitJson = await createResetGroupingVariant("filter")
  expect(circuitJson.filter((e) => e.type.endsWith("_error"))).toEqual([])
  expectReproNets(circuitJson, [
    ["U1.ADC_IN", "R1.pin1", "C1.pin1", "net.RESET"],
    ["R1.pin2", "net.SUPPLY"],
    ["C1.pin2", "net.GND"],
  ])
